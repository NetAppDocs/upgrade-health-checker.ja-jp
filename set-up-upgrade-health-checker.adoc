...
* ホスティング要件：VMは、クラスタ接続とインターネットアクセスが可能な場所に配置してください。これにより、ツールの自動更新が可能になります。自動更新を実行するために、マシンはHTTPS（443）経由で以下のエンドポイントにアクセスできる必要があります：
+
...
さらに、仮想マシンがHTTPS経由でテレメトリエンドポイント（+https://support.netapp.com+/および+https://api.activeiq.netapp.com+）に接続できることを確認し、NetAppがアップグレードプランに関するAutoSupport情報を受信できるようにしてください。
